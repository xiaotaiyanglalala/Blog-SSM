...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray"> <span class="l"> <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" href="javascript:;" onclick="admin_role_add('添加角色','<%=request.getContextPath() %>/role/add','800')"><i class="Hui-iconfont">&#xe600;</i> 添加角色</a> </span> <span class="r">共有数据：<strong><%=roles != null ? roles.size() : 0 %></strong> 条</span> </div>
	<table class="table table-border table-bordered table-hover table-bg">
		...
		%>
			<tr class="text-c">
				...
				<td>
				<% 
				List<AdminUser> adminUsers = role.getUsers();
				if(adminUsers != null && adminUsers.size() > 0){
					for(AdminUser adminUser : adminUsers) {
				%>
				<a href="#"><%=adminUser.getUsername() %>,</a><%}} %></td>
				
				<td><%=role.getIntroduce() %></td>
				<td class="f-14"><a title="编辑" href="javascript:;" onclick="admin_role_edit('角色编辑','<%=request.getContextPath() %>/role/add','1')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_role_del(this,'1')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
			<%}} %>
		</tbody>
	</table>
