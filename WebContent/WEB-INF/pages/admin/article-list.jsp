...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" data-title="添加文章" data-href="<%=request.getContextPath() %>/article/add" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加文章</a></span> <span class="r">共有数据：<strong><%=articles != null ? articles.size() : 0 %></strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			%>
				<tr class="text-c">
					...
					<td><%=article.getId() %></td>
					<td class="text-l"><u style="cursor:pointer" class="text-primary" onClick="article_edit('文章查看','<%=request.getContextPath() %>/article/look?id=<%=article.getId() %>','10001')" title="查看"><%=article.getTitle() %></u></td>
					<td><%=article.getAssort().getName() %></td>
					<td><%=article.getSource() %></td>
					<td><%=format.format(new Date(Long.valueOf(article.getCtime()))) %></td>
					<td><%=article.getLookNum() %></td>
					<td class="td-status"><span class="label label-success radius">已发布</span></td>
					<td class="f-14 td-manage"><a style="text-decoration:none" onClick="article_stop(this,'10001')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a> <a style="text-decoration:none" class="ml-5" onClick="article_edit('文章编辑','<%=request.getContextPath() %>/article/edit?id=<%=article.getId() %>','10001')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="article_del(this,'<%=article.getId() %>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
				<%}} %>
			</tbody>
		</table>
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<%=request.getContextPath()%>/article/del?id=' + id,
			dataType: 'json',
