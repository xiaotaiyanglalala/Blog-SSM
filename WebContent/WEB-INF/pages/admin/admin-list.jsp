...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="admin_add('添加管理员','<%=request.getContextPath() %>/adminuser/add','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a></span> <span class="r">共有数据：<strong><%=adminUsers != null ? adminUsers.size() : 0 %></strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td><%=adminUser.getUid() %></td>
				<td><%=adminUser.getUsername() %></td>
				<td><%=adminUser.getPhone() %></td>
				...
				<td><%=format.format(new Date(Long.valueOf(adminUser.getCrtime()))) %></td>
				<% if(adminUser.getState().equals("1")) {%>
				<td class="td-status"><span class="label label-success radius">已启用</span></td>
				<td class="td-manage"><a style="text-decoration:none" onClick="admin_stop(this,'<%=adminUser.getUid() %>')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> <a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑','<%=request.getContextPath() %>/adminuser/update?id=<%=adminUser.getUid() %>','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_del(this,'<%=adminUser.getUid() %>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				<%}else{%>
				<td class="td-status"><span class="label label-default radius">已停用</span></td>
				<td class="td-manage"><a style="text-decoration:none" onClick="admin_start(this,'<%=adminUser.getUid() %>')" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe615;</i></a> <a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑','<%=request.getContextPath() %>/adminuser/update?id=<%=adminUser.getUid() %>','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_del(this,'<%=adminUser.getUid() %>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				<%}%>
			</tr>
			<%	}
				} %>
...
function admin_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		...
		$.ajax({
			type : "POST",
			url : "<%=request.getContextPath()%>/adminuser/stop",
			data : "id=" + id,
			dataType : "json",
...
function admin_start(obj,id){
	layer.confirm('确认要启用吗？',function(index){
		...
		$.ajax({
			type : "POST",
			url : "<%=request.getContextPath()%>/adminuser/start",
			data : "id=" + id,
			dataType : "json",
			success : function(msg) {
				if(msg.ok == 1) {
